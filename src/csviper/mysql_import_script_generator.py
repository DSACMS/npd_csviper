...
class MySQLImportScriptGenerator:
    """
    ...
    def fromResourceDirToScript(resource_dir, output_dir=None, overwrite_previous=False):
        ...
        # Find metadata file
        metadata_file = MySQLImportScriptGenerator._find_metadata_file(resource_dir)
        ...
        # Validate required MySQL SQL files exist
        MySQLImportScriptGenerator._validate_mysql_sql_files(resource_dir, metadata)
        
        # Generate go.mysql.py script
        go_script_path = os.path.join(output_dir, 'go.mysql.py')
        
        if os.path.exists(go_script_path) and not overwrite_previous:
            raise FileExistsError(f"go.mysql.py already exists: {go_script_path}. Use overwrite_previous=True to overwrite.")
        
        script_content = MySQLImportScriptGenerator._generate_mysql_script_content(metadata)
    ...
    @staticmethod
    def _validate_mysql_sql_files(resource_dir, metadata):
    ...
    @staticmethod
    def _generate_mysql_script_content(metadata):
...
Generated on: {MySQLImportScriptGenerator._get_timestamp()}
"""
